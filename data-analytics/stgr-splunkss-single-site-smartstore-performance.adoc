...
SmartStore性能基于从大量数据中搜索特定模式和字符串。在此验证中、事件是使用生成的 https://["Eventgen"^] 通过搜索头访问特定Splunk索引(eventgen-test)、对于大多数查询、此请求将转至StorageGRID。下图显示了查询数据的命中和未命中情况。命中数据来自本地磁盘、未命中数据来自StorageGRID 控制器。
