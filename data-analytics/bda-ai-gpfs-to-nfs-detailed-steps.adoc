...
summary: 本节详细介绍了使用 NetApp XCP 配置 GPFS 并将数据移动到 NFS 所需的步骤。 
---
= 从 GPF 到 NFS 详细步骤
:hardbreaks:
:allow-uri-read: 
...
link:bda-ai-business-benefits.html["上一篇：业务优势。"]
...
. 在其中一台服务器上下载并安装适用于 Linux 的 Spectrum Scale Data Access 。
+
....
...
....
. 指定 GPFS 要使用的远程 shell 二进制文件；使用 ` -r 参数` 。
+
....
...
[root@mastr-51 installer]#
....
. 指定 GPFS 要使用的远程文件副本二进制文件；使用 ` -rc 参数` 。
+
....
...
[root@mastr-51 installer]#
....
. 指定要在所有 GPFS 节点上设置的端口范围；使用 ` -e 参数` 。
+
....
...
link:bda-ai-mapr-fs-to-ontap-nfs.html["接下来： MapR-FS 到 ONTAP NFS 。"]
