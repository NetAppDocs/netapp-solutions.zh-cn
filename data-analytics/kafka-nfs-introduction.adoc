...
虽然NFS上的正常Kafka操作运行良好、但 https://["重命名操作不明智"^] 在调整NFS上运行的Kafka集群的大小或重新分区期间、问题描述 会使应用程序崩溃。这是一个重要的问题描述 、因为出于负载平衡或维护目的、必须调整Kafka集群的大小或对其进行重新分区。您可以找到其他详细信息 https://["此处"^]。
