...
有关 VDS 日志文件的信息，请参见 https://["对 VDA 操作失败页面进行故障排除"^]。

有关所需最低权限的详细信息，请参见 https://["VDA 组件和权限页面"^]。

如果要手动克隆服务器，请参见 https://["克隆虚拟机页面"^]。

要自动增加 VM 磁盘大小，请参见 https://["自动增加磁盘空间功能页面"^]。

要确定用于手动配置客户端的网关地址，请参见 https://["最终用户要求页面"^]。
...
有关 NetApp Cloud Insights 的详细信息，请参见 https://["此视频"^]。
