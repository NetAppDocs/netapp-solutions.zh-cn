...
[role="lead"]
本解决方案 介绍了使用 Terraform 模块在 AWS （ CVO 单节点， CVO HA anf FSX ONTAP ）和 Azure （ CVO 单节点， CVO HA 和 ANF ）上自动部署云卷的情况。可在中找到此代码 https://github.com/NetApp-Automation/na_cloud_volumes_automation[]
...
| * AZ 容量池大小 * | 整型 | （必需）以 TB 为单位提及的 Capacy 池大小。 
...
为启用了数据保护的单个 ANF 卷提供不同的 Terraform 。
...
| * AZ-Capacity_Pool_size_primary_* | 整型 | （必需）以 TB 为单位提及的 Capacy 池大小。 


| * AZ 容量池大小二级 * | 整型 | （必需）以 TB 为单位提及的 Capacy 池大小。 
...
.CVO 单节点部署
--
本节包含用于在 Azure 上部署 / 配置单节点 CVO （ Cvoud Volumes ONTAP ）的各种 Terraform 配置文件。
...
| * AZ-CVO_OITE* | string | （必需）要创建工作环境的位置。 
