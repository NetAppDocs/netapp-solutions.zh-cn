...
IAM为Cloud Volumes Service 提供内置的粒度权限。您可以找到 https://cloud.google.com/architecture/partners/netapp-cloud-volumes/security-considerations["在此填写粒度权限列表"^]。
...
请参见 https://cloud.google.com/architecture/partners/netapp-cloud-volumes/api["正在创建服务帐户和私钥"^] 有关详细信息、请参见Google云文档。



== Cloud Volumes Service API

Cloud Volumes Service API使用基于REST的API、并使用HTTPS (TLSv1.2)作为底层网络传输。您可以找到最新的API定义 https://cloudvolumesgcp-api.netapp.com/swagger.json["此处"^] 以及有关如何使用API的信息、请参见 https://cloud.google.com/architecture/partners/netapp-cloud-volumes/api["Google云文档中的Cloud Volumes API"^]。
