...
使用LDAP和Kerberos时、您应确定这些服务正在使用的网络端口。您可以在中找到Cloud Volumes Service 正在使用的端口的完整列表 https://cloud.google.com/architecture/partners/netapp-cloud-volumes/security-considerations["有关安全注意事项的Cloud Volumes Service 文档"^]。
