...
[role="lead"]
适用于Google Cloud的Cloud Volumes Service 利用了Google Cloud https://["私有服务访问"^] 框架。在此框架中、用户可以连接到Cloud Volumes Service。此框架像使用其他Google Cloud服务一样使用服务网络和VPC对等结构、确保租户之间完全隔离。

有关适用于Google Cloud的Cloud Volumes Service 架构概述、请参见 https://["适用于Cloud Volumes Service 的架构"^]。
