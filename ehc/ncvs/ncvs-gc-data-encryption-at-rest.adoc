...
自2021年11月起、提供了预览客户管理的加密密钥(CMEK)功能。这样、您就可以使用中托管的每个项目的每个区域主密钥对每个卷的密钥进行加密 https://["Google密钥管理服务(KMS)。"^] 您可以通过Kms连接外部密钥管理器。

有关为KMS配置CVS-Performance的信息、请参见 https://["设置客户管理的加密密钥"^]。
