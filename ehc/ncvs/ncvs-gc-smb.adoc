...
请记住、每个区域和服务类型仅允许一个Active Directory连接。
...
| LM兼容性级别 | Active Directory连接支持的身份验证方言级别。请参见第节" authentication dialects"了解更多信息。 | NTLMv2-KRB | 否 


| 传入CIFS流量需要SMB加密 | 所有共享都需要SMB加密。Cloud Volumes Service 不会使用此功能；而是按卷设置加密(请参见一节 share security features")。 | false | 否 
