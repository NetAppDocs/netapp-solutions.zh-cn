...
要在Azure上配置AVS私有云、请按照中的步骤进行操作 https://docs.netapp.com/us-en/netapp-solutions/ehc/azure-setup.html["链接。"^] 适用于NetApp文档和本 https://learn.microsoft.com/en-us/azure/azure-vmware/deploy-azure-vmware-solution?tabs=azure-portal["链接。"^] 了解Microsoft文档。  采用最低配置设置的指示灯环境可用于灾难恢复。  此设置仅包含支持关键应用程序的核心组件、并且可以横向扩展并生成更多主机、以便在发生故障转移时承担大部分负载。
