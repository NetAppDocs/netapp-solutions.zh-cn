...
image::dr-veeam-anf-image1.png[灾难恢复Veeam和image1]
...
+
image::dr-veeam-anf-image2.png[DR Veeam和image2]
...
+
image::dr-veeam-anf-image3.png[灾难恢复Veeam和image3]

. 之后、选择目标作为Azure VMware解决方案SDDC集群/主机、并为VM副本选择相应的资源池、VM文件夹和FSx for ONTAP数据存储库。然后单击 * 下一步 * 。
+
image::dr-veeam-anf-image4.png[灾难恢复Veeam和image4]

. 在下一步中、根据需要创建源虚拟网络与目标虚拟网络之间的映射。
+
image::dr-veeam-anf-image5.png[灾难恢复Veeam和image5]
...
+
image::dr-veeam-anf-image6.png[灾难恢复Veeam和image6]

. 选择复制计划以定期运行复制作业。
+
image::dr-veeam-anf-image7.png[灾难恢复Veeam和image7]

. 在向导的*摘要*步骤中，查看复制作业的详细信息。要在关闭向导后立即启动作业，请选中*单击完成时运行作业*复选框，否则不要选中该复选框。然后单击*完成*关闭向导。
+
image::dr-veeam-anf-image8.png[灾难恢复Veeam和image8]



复制作业启动后、目标AVS SDDC集群/主机上将填充具有指定后缀的VM。

image::dr-veeam-anf-image9.png[灾难恢复Veeam和image9]
...
+
image::dr-veeam-anf-image10.png[灾难恢复Veeam和image10]

. 接下来、提供计划的名称和问题描述。可以根据需要添加故障转移前和故障转移后脚本。例如、在启动复制的VM之前、请运行一个脚本来关闭VM。
+
image::dr-veeam-anf-image11.png[Veeam和image11]

. 将VM添加到计划中、并修改VM启动顺序和启动延迟、以满足应用程序依赖关系。
+
image::dr-veeam-anf-image12.png[DR Veeam和image12]
...
image::dr-veeam-anf-image13.png[Veeam和image13]

image::dr-veeam-anf-image14.png[Veeam和image14]

VM副本的状态将从"准备就绪"更改为"故障转移"、VM将在目标Azure VMware解决方案(AVS) SDDC集群/主机上启动。

image::dr-veeam-anf-image15.png[灾难恢复Veeam和image15]

故障转移完成后、VM的状态将更改为"故障转移"。

image::dr-veeam-anf-image16.png[DR Veeam和image16]
...
image::dr-veeam-anf-image17.png[Veeam和image17]

image::dr-veeam-anf-image18.png[Veeam和image18]

image::dr-veeam-anf-image19.png[Veeam博士和版本19]

image::dr-veeam-anf-image20.png[灾难恢复Veeam和image20]
...
image::dr-veeam-anf-image21.png[Veeam和image21]

成功故障恢复到生产环境后、所有VM都会还原回原始生产站点。

image::dr-veeam-anf-image22.png[Veeam和image22]
