...
+
您可以通过单击HCX版本链接image：：gcpd-HCX-image2.png[HCX Console access with link on GCVE resource]或单击"vSphere Management Network"(vSphere管理网络)选项卡下的HCX FQDN来登录到HCX控制台。 图像：：gcpd-HCX-image3.png[HCX控制台访问和FQDN链接]

. 在HCX Cloud Manager中、转到*管理>系统更新*。
. 单击*请求下载链接*并下载OVA文件。 图像：：gcpd-HCX-image4.png[请求下载链接]
...
. 在VMware Engine门户中，单击Resources，选择私有云，然后*单击HCX Manager Cloud Version*下的下载图标。 image::gcpd-HCX-image6.png[下载HCX License]打开下载的文件并复制许可证密钥字符串。
