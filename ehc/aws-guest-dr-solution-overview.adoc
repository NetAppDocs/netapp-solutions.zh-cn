...
image::dr-vmc-aws-image1.png[灾难恢复vmc AWS image1]
...
+
image::dr-vmc-aws-image10.png[灾难恢复vmc AWS image10]
...
....
+
image::dr-vmc-aws-image11.png[灾难恢复vmc AWS image11]
...
+
image::dr-vmc-aws-image12.png[灾难恢复vmc AWS image12]
...
.. 单击启动集群对等以完成此过程。
+
image::dr-vmc-aws-image13.png[灾难恢复vmc AWS image13]
...
....
+
image::dr-vmc-aws-image14.png[灾难恢复vmc AWS image14]
...
+
image::dr-vmc-aws-image15.png[灾难恢复vmc AWS image15]
...
** 目标Storage VM
+
image::dr-vmc-aws-image16.png[灾难恢复vmc AWS image16]
...
image::dr-vmc-aws-image17.png[灾难恢复vmc AWS image17]
...
+
image::dr-vmc-aws-image21.png[灾难恢复vmc AWS image21]

. 对于Oracle、请在添加主机对话框中填写必填字段、然后选中Oracle数据库插件对应的复选框。然后、单击提交开始发现过程、并将主机添加到SnapCenter。
+
image::dr-vmc-aws-image22.png[灾难恢复vmc AWS image22]
...
image::dr-vmc-aws-image23.png[灾难恢复vmc AWS image23]
...
image::dr-vmc-aws-image24.png[灾难恢复vmc AWS image24]
...
image::dr-vmc-aws-image25.png[灾难恢复vmc AWS image25]
...
+
image::dr-vmc-aws-image26.png[灾难恢复vmc AWS image26]

. 在添加存储向导中、选择NetApp作为存储供应商、然后选择Data ONTAP。
. 输入管理IP地址并选中NAS文件器复选框。单击下一步。
+
image::dr-vmc-aws-image27.png[灾难恢复vmc AWS image27]

. 添加凭据以访问ONTAP 集群。
+
image::dr-vmc-aws-image28.png[灾难恢复vmc AWS image28]

. 在NAS文件管理器页面上、选择要扫描的协议、然后选择下一步。
+
image::dr-vmc-aws-image29.png[灾难恢复vmc AWS image29]

. 完成向导的"Apply"和"Summary"页面、然后单击"Finish"开始存储发现过程。扫描完成后、ONTAP 集群将与NAS存储器一起添加为可用资源。
+
image::dr-vmc-aws-image30.png[灾难恢复vmc AWS image30]

. 使用新发现的NAS共享创建备份存储库。从备份基础架构中、选择备份存储库、然后单击添加存储库菜单项。
+
image::dr-vmc-aws-image31.png[灾难恢复vmc AWS image31]

. 按照"新建备份存储库向导"中的所有步骤创建存储库。有关创建Veeam备份存储库的详细信息、请参见 https://www.veeam.com/documentation-guides-datasheets.html["Veeam文档"^]。
+
image::dr-vmc-aws-image32.png[灾难恢复vmc AWS image32]
...
+
image::dr-vmc-aws-image33.png[灾难恢复vmc AWS image33]

. 在添加备份存储库向导中、选择对象存储、然后选择Amazon S3。此时将启动"新建对象存储库"向导。
+
image::dr-vmc-aws-image34.png[灾难恢复vmc AWS image34]

. 提供对象存储库的名称、然后单击下一步。
. 在下一节中、提供您的凭据。您需要AWS访问密钥和机密密钥。
+
image::dr-vmc-aws-image35.png[灾难恢复vmc AWS image35]
...
+
image::dr-vmc-aws-image40.png[灾难恢复vmc AWS image40]

. 展开"Auth"部分、然后单击Try it out。
+
image::dr-vmc-aws-image41.png[灾难恢复vmc AWS image41]

. 在用户操作文本区域中、填写SnapCenter 凭据和角色、然后单击执行。
+
image::dr-vmc-aws-image42.png[灾难恢复vmc AWS image42]

. 在下面的响应正文中、您可以看到令牌。执行备份过程时、复制令牌文本以进行身份验证。
+
image::dr-vmc-aws-image43.png[灾难恢复vmc AWS image43]
...
+
image::dr-vmc-aws-image44.png[灾难恢复vmc AWS image44]

. 展开`/4.6/disasterrecovery/server/backup`部分、然后单击Try it out。
+
image::dr-vmc-aws-image45.png[灾难恢复vmc AWS image45]
...
+
image::dr-vmc-aws-image46.png[灾难恢复vmc AWS image46]
...
image::dr-vmc-aws-image47.png[灾难恢复vmc AWS image47]
...
+
image::dr-vmc-aws-image48.png[灾难恢复vmc AWS image48]

. 粘贴您的授权令牌、然后在"SmDRResterRequest"部分中、将备份名称和二级SnapCenter 服务器上的本地目录粘贴。
+
image::dr-vmc-aws-image49.png[灾难恢复vmc AWS image49]

. 选择执行按钮以启动还原过程。
. 在SnapCenter 中、导航到Monitor部分以查看还原作业的进度。
+
image::dr-vmc-aws-image50.png[灾难恢复vmc AWS image50]

+
image::dr-vmc-aws-image51.png[灾难恢复vmc AWS image51]
...
+
image::dr-vmc-aws-image52.png[灾难恢复vmc AWS image52]
...
+
image::dr-vmc-aws-image53.png[灾难恢复vmc AWS image53]

. 选择Amazon S3作为对象存储类型。
+
image::dr-vmc-aws-image54.png[灾难恢复vmc AWS image54]

. 从Amazon Cloud Storage Services列表中、选择Amazon S3。
+
image::dr-vmc-aws-image55.png[灾难恢复vmc AWS image55]

. 从下拉列表中选择预先输入的凭据、或者添加用于访问云存储资源的新凭据。单击下一步继续。
+
image::dr-vmc-aws-image56.png[灾难恢复vmc AWS image56]

. 在存储分段页面上、输入数据中心、存储分段、文件夹以及任何所需选项。单击应用。
+
image::dr-vmc-aws-image57.png[灾难恢复vmc AWS image57]
...
+
image::dr-vmc-aws-image58.png[灾难恢复vmc AWS image58]

. 为导入创建数据库记录后、在摘要屏幕上选择下一步、然后选择完成以启动导入过程。
+
image::dr-vmc-aws-image59.png[灾难恢复vmc AWS image59]

. 导入完成后、您可以将虚拟机还原到VMware Cloud集群中。
+
image::dr-vmc-aws-image60.png[灾难恢复vmc AWS image60]
...
+
image::dr-vmc-aws-image61.png[灾难恢复vmc AWS image61]

. 在完整虚拟机还原向导的第一页上、根据需要修改要备份的虚拟机、然后选择下一步。
+
image::dr-vmc-aws-image62.png[灾难恢复vmc AWS image62]

. 在还原模式页面上、选择还原到新位置或使用不同设置。
+
image::dr-vmc-aws-image63.png[灾难恢复vmc AWS image63]

. 在主机页面上、选择要将虚拟机还原到的目标ESXi主机或集群。
+
image::dr-vmc-aws-image64.png[灾难恢复vmc AWS image64]

. 在Datastores页面上、为配置文件和硬盘选择目标数据存储库位置。
+
image::dr-vmc-aws-image65.png[灾难恢复vmc AWS image65]

. 在网络页面上、将虚拟机上的原始网络映射到新目标位置中的网络。
+
image::dr-vmc-aws-image66.png[灾难恢复vmc AWS image66]

+
image::dr-vmc-aws-image67.png[灾难恢复vmc AWS image67]
...
+
image::dr-vmc-aws-image68.png[灾难恢复vmc AWS image68]

. 在发现选项卡中、单击发现门户、然后输入FSX iSCSI目标的IP地址。
+
image::dr-vmc-aws-image69.png[灾难恢复vmc AWS image69]

+
image::dr-vmc-aws-image70.png[灾难恢复vmc AWS image70]

. 在目标选项卡上、单击连接、根据您的配置选择启用多路径、然后单击确定连接到目标。
+
image::dr-vmc-aws-image71.png[灾难恢复vmc AWS image71]

. 打开计算机管理实用程序并使磁盘联机。确认它们保留的驱动器号与先前相同。
+
image::dr-vmc-aws-image72.png[灾难恢复vmc AWS image72]
...
+
image::dr-vmc-aws-image73.png[灾难恢复vmc AWS image73]

. 单击添加并导航到包含SQL Server主数据库文件的文件夹、将其选中、然后单击确定。
+
image::dr-vmc-aws-image74.png[灾难恢复vmc AWS image74]

. 如果事务日志位于单独的驱动器上、请选择包含事务日志的文件夹。
. 完成后、单击确定以连接数据库。
+
image::dr-vmc-aws-image75.png[灾难恢复vmc AWS image75]
...
image::dr-vmc-aws-image76.png[灾难恢复vmc AWS image76]
...
....
+
image::dr-vmc-aws-image77.png[灾难恢复vmc AWS image77]
...
....
+
image::dr-vmc-aws-image78.png[灾难恢复vmc AWS image78]

. 更新Amazon FSX Web客户端中的接合路径：
+
image::dr-vmc-aws-image79.png[灾难恢复vmc AWS image79]

. 添加接合路径名称、然后单击更新。从Oracle服务器挂载NFS卷时、请指定此接合路径。
+
image::dr-vmc-aws-image80.png[灾难恢复vmc AWS image80]
...
+
image::dr-vmc-aws-image81.png[灾难恢复vmc AWS image81]

. 从操作菜单中、选择挂载命令以查看并复制要在Oracle Linux服务器上使用的挂载命令。
+
image::dr-vmc-aws-image82.png[灾难恢复vmc AWS image82]

+
image::dr-vmc-aws-image83.png[灾难恢复vmc AWS image83]
