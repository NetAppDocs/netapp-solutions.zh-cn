...
. 您已有一个有效的 Kubernetes 集群，并且正在运行 Kubernetes 支持的版本。有关支持的版本列表，请参见 https://["Kubeflow 官方文档"^]。
...
NOTE: 或者，您也可以按照手动方式部署 Kubeflow https://["安装说明"^] 在 Kubeflow 官方文档中

. 按照中的说明在集群中部署 Kubeflow https://["Kubeflow 部署说明"^] 在 NVIDIA DeepOps GitHub 站点上。
...
+
默认用户名为 `admin@kubeflow.org` ，默认密码为 `1231234` 。要创建其他用户，请按照中的说明进行操作 https://["Kubeflow 官方文档"^]。
