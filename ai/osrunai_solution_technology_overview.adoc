...
此解决方案已使用 ONTAP AI 参考架构两个 DGX-1 节点和一个 AFF A800 存储系统进行了验证。请参见 https://["NVA-1121"^] 有关此验证中使用的基础架构的更多详细信息。
...
此解决方案已通过安装了 Run ： AI 操作员的基本 Kubernetes 部署进行验证。Kubernetes 是使用部署的 https://["NVIDIA DeepOps"^] 部署引擎，用于为生产就绪环境部署所有必需的组件。DeepOps 会自动部署 https://["NetApp Trident"^] 为了与 K8s 环境实现持久存储集成，我们创建了默认存储类，以便容器可以利用 AFF A800 存储系统中的存储。有关在 ONTAP AI 上使用 Kubernetes 进行 Trident 的详细信息，请参见 https://["TR-4798"^]。
...
|===
有关 Run ： AI 的其他软件要求，请参见 https://["运行： AI GPU 集群前提条件"^]。
