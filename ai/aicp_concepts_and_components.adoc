...
link:https://docs.netapp.com/us-en/trident/index.html["Trident"^] 支持在所有常见的NetApp存储平台上使用和管理存储资源、无论是公有云还是内部环境、包括ONTAP (AFF、FAS、Select、Cloud、 Amazon FSx for NetApp ONTAP)、Azure NetApp Files服务和Google Cloud NetApp卷。Trident是一款符合容器存储接口(CSI)的动态存储编排程序、可与Kubnetes本机集成。
