...
link:xcp-bp-migration-workflow-overview.html["上一步：迁移工作流。"]
...
image:xcp-bp_image6.png["错误：缺少图形映像"]


NOTE: 有关 XCP 文件分析的高级架构，统计信息视图等基于图形用户界面的信息板视图以及文件分发视图详细信息，请参见 https://blog.netapp.com/xcp-data-migration-software["NetApp XCP 1.6 改进了开放式文件分析和基础架构"^] 博客

在 XCP 1.6 中，用于自定义图形的图形用户界面有限。要创建所需的图形，您可以使用命令行界面，使用匹配的筛选器运行 `XCP scan` 命令。请参见以下示例。
...
image:xcp-bp_image7.png["错误：缺少图形映像"]

link:xcp-bp-deployment-steps.html["下一步：部署步骤。"]
