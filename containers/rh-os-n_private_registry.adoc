...
---
= 创建私有映像注册表：使用 NetApp 的 Red Hat OpenShift
:hardbreaks:
:nofooter: 
:icons: font
:linkattrs: 
...
对于大多数 Red Hat OpenShift 部署，请使用等公有注册表 https://quay.io["Quay.io"] 或 满足大多数客户的需求。但是，有时客户可能希望托管自己的私有或自定义映像。

本操作步骤介绍了如何创建私有映像注册表，该注册表由 Astra Trident 和 NetApp ONTAP 提供的永久性卷提供支持。


NOTE: Astra 控制中心需要注册表来托管 Astra 容器所需的映像。以下部分介绍了在 Red Hat OpenShift 集群上设置专用注册表以及推送支持安装 Astra 控制中心所需的映像的步骤。



== 创建私有映像注册表

. 编辑 imageeregistry 运算符，在 `sPec` 部分输入以下存储参数
+
[listing]
----
...
storage:
  pvc:
    claim:
----
. 然后，在 `sPec` 部分输入以下参数，使用自定义主机名创建 OpenShift 路由，保存并退出
+
[listing]
----
routes:
...
NOTE: 如果要为路由设置自定义主机名，则会使用上述路由配置。如果您希望 OpenShift 使用默认主机名创建路由，您只需将以下参数添加到 `sPec` section – `defaultRoute ： true`

+
.自定义 TLS 证书
****
默认情况下，当您对路由使用自定义主机名时，它会使用 OpenShift 入口操作员的默认 TLS 配置。但是，您可以向路由添加自定义 TLS 配置。为此，请执行以下步骤—
...
****
. 下一步是再次编辑 imageeregistry 运算符，并将该运算符的管理状态更改为 `Maned` state ， save and exit –
+
[listing]
----
...
----
. 如果满足所有前提条件，则应开始为专用映像注册表创建 PVC ， Pod 和服务。几分钟后，注册表就会启动。
+
[listing]
----
...
----
. 如果您使用的是传入操作员 OpenShift 注册表路由的默认 TLS 证书，则可以使用以下命令提取 TLS 证书。
+
[listing]
----
[netapp-user@rhel7 ~]$ oc extract secret/router-ca --keys=tls.crt -n openshift-ingress-operator
----
. 要允许 OpenShift 节点访问并从注册表中提取映像，您需要将证书添加到 OpenShift 节点上的 Docker 客户端。使用 TLS 证书在 `OpenShift-config` 命名空间中创建一个配置映射，并将其修补到集群映像配置中以使此证书可信。
+
[listing]
----
...
+
.. 要允许用户 / 用户组从注册表中提取映像，必须为用户分配注册表查看器角色。
+
[listing]
----
...
----
.. 要允许用户 / 用户组写入或推送映像，必须为用户分配注册表编辑器角色。
+
[listing]
----
...
. 要使 OpenShift 节点能够访问注册表并推 / 拉映像，您需要配置拉取密钥。
+
[listing]
----
...
----
.. 要在 Pod 定义中引用拉取密钥，请将以下参数添加到 ‘sPec ' 部分。
+
[listing]
----
imagePullSecrets:
...
----
.. 使用 OpenShift 用户凭据通过 podman/Docker 命令登录到注册表。
+
[role="tabbed-block"]
====
.podman
