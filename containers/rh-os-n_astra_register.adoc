...
NOTE: 由 Astra 控制中心管理的所有 Red Hat OpenShift 集群都应有权访问用于安装的映像注册表，因为受管集群上安装的代理将从该注册表中提取映像。
...
link:rh-os-n_astra_applications.html["接下来：选择要保护的应用程序：采用 NetApp 的 Red Hat OpenShift 。"]
