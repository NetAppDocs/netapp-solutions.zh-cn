...
summary: 本参考文档对通过安装程序配置的基础架构（ IPI ）在 NetApp 验证的多种不同数据中心环境中部署的 Red Hat OpenShift 解决方案进行了部署验证。同时，还详细介绍了如何利用 NetApp Trident 存储编排程序来管理永久性存储，从而与 NetApp 存储系统实现存储集成。最后，我们还探讨并记录了许多解决方案验证和实际使用情形。 
---
...
* 与 NetApp 存储和适用于 Kubernetes 的开源存储编排程序 NetApp Trident 结合使用时，重点介绍 Red Hat OpenShift 功能的实际配置和使用情形。
...
采用 NetApp 解决方案的 Red Hat OpenShift 由以下三个组件组成：



=== Red Hat OpenShift 容器平台

Red Hat OpenShift 容器平台是一个完全受支持的企业级 Kubernetes 平台。 Red Hat 对开源 Kubernetes 进行了多项增强，以提供一个应用程序平台，其中包含所有组件，可完全集成以构建，部署和管理容器化应用程序。
...
有关详细信息，请访问 Trident 网站 https://netapp-trident.readthedocs.io/en/stable-v21.04/index.html["此处"]。
