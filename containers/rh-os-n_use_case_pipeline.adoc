...
. 在左上角，将角色从管理员更改为开发人员。单击 ` + 添加` ，然后从目录` 中选择 `。在 `Filter by Keyword` 栏中，搜索 Jenkins 。选择 Jenkins Service with Persistent Storage 。
+
...
. 实例化 Pod 后，导航到 `Networking > routes` 。要打开 Jenkins 网页，请单击为 Jenkins 路由提供的 URL 。
+
image:redhat_openshift_image21.png["错误：缺少图形映像"]

. 由于在创建 Jenkins 应用程序时使用了 OpenShift OAuth ，因此请单击 `Log in with OpenShift` 。
+
...
. 现在，您可以创建一个管道来演示 CI/CD 工作流。在主页上，从左侧菜单中单击 `Create New Jobs` 或 `New Item` 。
+
...
. 单击 `立即构建` 可在准备，构建和测试阶段触发开发。完成整个构建过程并显示构建结果可能需要几分钟的时间。
+
