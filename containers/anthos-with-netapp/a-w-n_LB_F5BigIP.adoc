...
---
= 安装F5 BIG-IP负载平衡器：采用NetApp的Anthos
:allow-uri-read: 
...
F5 BIG-IP可以通过多种方式进行部署和使用、包括在专用硬件上、在云中或作为内部虚拟设备。请参见此处的文档，了解如何根据需要部署 F5 BIG-IP 。
...
. 显示的屏幕仅显示当前通用分区。单击右侧的Create创建以创建第一个附加分区、并将其命名为`GKE-Admin`。然后、单击Repeat并将分区命名为`User-Cluster-1`、然后再次单击Repeat按钮将下一个分区命名为`User-Cluster-2`。最后，单击 " 完成 " 以完成向导。此时将返回分区列表屏幕，其中列出了所有分区。
+
...
下面是GKE-Admin集群分区配置示例、需要取消注释和修改的值将以粗体文本显示：
