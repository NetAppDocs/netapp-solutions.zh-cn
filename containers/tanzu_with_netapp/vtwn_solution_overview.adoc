...
* vSphere原生 Pod在基于光子的精简层中运行、并使用规定的虚拟硬件进行完全隔离。
* 需要NSX-T、但这样可以支持更多功能、例如、存储映像注册表。
* 使用TKGS等虚拟Supervisor集群在vSphere 7.0U1中部署和管理、并直接在ESXi节点上运行Pod。
* 完全集成vSphere、由vSphere管理员提供最高的可见性和控制能力。
* 基于CRX的隔离POD、可实现最高的安全性。
* 仅支持将vSphere CSI用于永久性存储/不支持第三方存储编排程序。
