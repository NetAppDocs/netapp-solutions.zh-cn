...
+
image::sql-awsec2-image1.png[SQL awsec2 image1]

. 指定fsxadmin的密码。
+
image::sql-awsec2-image2.png[SQL awsec2 image2.]

. 指定SVM的密码。
+
image::sql-awsec2-image3.png[SQL awsec2 image3.]
...
....
+
image::sql-awsec2-image5.png[SQL awsec2 image5.]
...
+
image::sql-awsec2-image8.png[SQL awsec2 image8.]

. 对SVM中的第二个iSCSI IP重复步骤12。
. 选择*目标*选项卡、选择*连接*、然后选择*启用多路径*。
+
image::sql-awsec2-image9.png[SQL awsec2 image9.]
...
....
+
image::sql-awsec2-image10.png[SQL awsec2 image10]
...
....
+
image::sql-awsec2-image11.png[SQL awsec2 image11]
...
....
+
image::sql-awsec2-image12.png[SQL awsec2 image12.]
...
image::sql-awsec2-image13.png[SQL awsec2 image13.]
...
+
image::sql-awsec2-image14.png[SQL awsec2 image14.]

. 通过选择存储系统>新建并为ONTAP 存储信息提供适当的FSX来添加存储系统。
+
image::sql-awsec2-image15.png[SQL awsec2 image15.]

. 通过选择*主机*>*添加*来添加主机、然后提供主机信息。SnapCenter 会自动安装Windows和SQL Server插件。此过程可能需要一些时间。
+
image::sql-awsec2-image16.png[SQL awsec2 image16.]
...
+
image::sql-awsec2-image17.png[SQL awsec2 image17.]

+
如果已配置LUN、则可以选择挂载点来表示主机日志目录。

+
image::sql-awsec2-image18.png[SQL awsec2 image18.]
...
+
image::sql-awsec2-image19.png[SQL awsec2 image19.]

.. 选择要备份的数据库，然后选择*Next*和(**)以添加策略(如果尚未创建)。按照*新SQL Server备份策略*创建新策略。
+
image::sql-awsec2-image20.png[SQL awsec2 image20.]

.. 如有必要、选择验证服务器。此服务器是SnapCenter 在创建完整备份后运行DBCC CHECKDB的服务器。单击*下一步*进行通知、然后选择*摘要*进行查看。查看后、单击*完成*。
+
image::sql-awsec2-image21.png[SQL awsec2 image21.]

.. 单击*立即备份*以测试备份。在弹出窗口中、选择*备份*。
+
image::sql-awsec2-image22.png[SQL awsec2 image22.]

.. 选择*监控*以验证备份是否已完成。
+
image::sql-awsec2-image23.png[SQL awsec2 image23.]
...
. 选择资源、然后选择要还原的数据库。
+
image::sql-awsec2-image24.png[SQL awsec2 image24]

. 选择需要从中还原数据库的备份名称、然后选择还原。
. 按照*还原*弹出窗口还原数据库。
. 选择*监控*以验证还原过程是否成功。
+
image::sql-awsec2-image25.png[SQL awsec2 image25.]
...
. 选择要克隆的备份名称、然后选择克隆。
+
image::sql-awsec2-image26.png[SQL awsec2 image26.]

. 按照"从备份克隆"说明、在EC2上提供新的SQL Server实例和实例名称、完成克隆过程。
. 选择Monitor以确保克隆已完成。
+
image::sql-awsec2-image27.png[SQL awsec2 image27.]
