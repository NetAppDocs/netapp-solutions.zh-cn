...
作者：Jsh Powell、Sresh Thoppay

本文档提供了有关使用NetApp全闪存SAN阵列的VMware Cloud Foundation可用存储选项的信息。有关将VMFS数据存储库(FC/iSCSI)部署为管理和工作负载域的主体存储和补充存储(FC/iSCSI/NVMe-oF)的具体说明、介绍了受支持的存储选项。此外、还介绍了使用适用于VMware vSphere的SnapCenter对虚拟机和数据存储库进行数据保护的情况。
...
VMware将VCF的存储选项分为*主体*和*补充*存储。从VCF 5.2开始、ONTAP存储可用作管理和工作负载域的主存储和补充存储、VCF 5.2可以选择将现有vSphere环境转换为VCF管理域或作为工作负载域导入到现有VCF环境。

image:vmware-vcf-aff-image01.png["VCF存储选项"]

*用于管理和工作负载的主要存储域*主要存储是指可以在域设置过程中部署或导入的任何类型的存储。要在管理域中使用ONTAP存储作为主存储、必须从现有vSphere环境进行转换。支持FC或NFS v3数据存储库上的VMFS选项。
...
| FC上的VMFS | *导入工具* | *SDDC API* | 是的。 | 是的。 


| iSCSI上的VMFS | 不适用 | 不适用 | 不适用 | 不适用 


| 使用FC的VVOC | 不适用 | 不适用 | 否 | 是的。 


| 使用iSCSI的vVol | 不适用 | 不适用 | 否 | 是的。 
|===
* 不适用—VCF不支持。
...
[cols="25% 25% 25% 25%"]
|===
| 存储类型 | 管理域-默认集群 | 管理域—附加集群 | VI工作负载域 


| FC上的VMFS | 是的。 | 是的。 | 是的。 


| iSCSI上的VMFS | 是的。 | 是的。 | 是的。 


| 使用FC的VVOC | 是的。 | 是的。 | 是的。 


| 使用iSCSI的vVol | 是的。 | 是的。 | 是的。 
|===
...
本文档中介绍的方案将演示如何使用ONTAP存储系统作为管理和工作负载域的主存储和补充存储。此外、适用于VMware vSphere的SnapCenter插件用于保护虚拟机和数据存储库。
...
* *部署并使用适用于VMware vSphere的SnapCenter插件来保护和还原VI工作负载域中的虚拟机*单击可查看部署步骤。link:vmware_vcf_asa_scv_wkld.html["*此处*"]
* *将延伸型集群与管理域中的VMFS数据存储库结合使用*单击link:vmware_vcf_asa_mgmt_stretchcluster.html["*此处*"]可查看部署步骤。
* *将延伸型集群与VI工作负载域中的VMFS数据存储库结合使用*单击link:vmware_vcf_asa_wkld_stretchcluster.html["*此处*"]可查看部署步骤。
