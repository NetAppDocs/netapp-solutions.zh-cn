...
在AWS中、FSx for NetApp ONTAP (FSxN)可以部署在单个可用性区域(AZ)或多个可用性区域(AZ)中。对于需要高可用性的生产工作负载、与单个AZ相比、多可用性可提供分区级容错、并具有更好的NVMe读取缓存。有关详细信息、请查看 link:https://docs.aws.amazon.com/fsx/latest/ONTAPGuide/performance.html["AWS性能准则"]。为了节省灾难恢复站点的成本、可以使用一个AZ FSx ONTAP。

image:rhhc_storage_options_fsxn_options.png["在多可用性分区和单可用性分区之间进行复制"]
